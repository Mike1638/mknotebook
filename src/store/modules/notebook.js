...
const getters = {
  ...
  currentBook: state => {
    ...
    return (
      state.notebooksList.find(
        notebook => notebook.id == state.currentBookId
      ) || {}
    );
  }
};
...
const mutations = {
  ...
  updateNotebook(state, payload) {
    let notebook =
      state.notebooksList.find(notebook => notebook.id == payload.notebookId) ||
      {};
    notebook.title = payload.title;
  },
  deleteNotebook(state, payload) {
    state.notebooksList = state.notebooksList.filter(
...
const actions = {
  ...
  updateNotebook({ commit }, payload) {
    return Notebooks.updateNotebook(payload.notebookId, {
      title: payload.title
    })
      .then(res => {
